...
const JumblingConfig = () =>{
    ...
    const fetchProgConfig = async () => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post(`${baseUrl}/api/ProgConfigs`,{ headers: { Authorization: `Bearer ${keygenUser?.token}` } }, formData);
          if (response.status !== 201) {
            throw new Error('Failed to submit form');
          }
                      ...
                      <Col>
                        <Form.Group controlId="programmeID">
                          ...
                          <Form.Control as="select" value={formData.progID} onChange={(e) => handleInputChange('progID', e.target.value)} required>
                            <option value="">Select a Program</option>
                            {programme.map((prog) => (
                              <option key={prog.progID} value={prog.progID}>{prog.programmeName}</option>
                            ))}
                          </Form.Control>
                        </Form.Group>
                      </Col>
